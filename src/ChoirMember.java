...
/**
 * The ChoirMember Class: This class is a thread that plays the note being passed to it by the ChoirConductor.
 * When the note is received and a signal from the ChoirCondutor is given to play the note,
 * the ChoirMember plays the note and then waits for another note to be passed to them.
...
public class ChoirMember implements Runnable {
     ...
     * This allows it to be found easily when compared to the note to be played.
     ...
     * If it is not the members, it will wait until it is notified to play.
     ...
     * the audible sound for that note.
     */
    ...
    /**
     * This method is used to have the memberStop playing and wait for the Song to finish before the program exits.
     */
    ...
    /**
     * The run method continues to call the play method until the time to play is false,
     * and the member is not playing the note at which time the member informs the user
     * it is done playing its part of the song.
     */
    public void run() {
        synchronized (this) {
            do {
